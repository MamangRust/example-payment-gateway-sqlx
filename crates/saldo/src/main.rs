...
use genproto::saldo::saldo_service_server::SaldoServiceServer;
use saldo::{
    config::ServerConfig,
    service::{SaldoServiceImpl, SaldoStats},
    state::AppState,
};
use shared::{
...
async fn main() -> Result<()> {
    ...
    let _logger = Logger::new("saldo-service", is_dev);
    ...
    let stats = SaldoStats {
        balance: state.di_container.saldo_stats.balance_service.clone(),
        total: state.di_container.saldo_stats.total_service.clone(),
    };

    let service = SaldoServiceImpl::new(
        state.di_container.saldo_query.service.clone(),
        state.di_container.saldo_command.service.clone(),
        stats,
    );
...
async fn start_grpc_server(
    ...
    tonic::transport::Server::builder()
        .add_service(SaldoServiceServer::new(service))
