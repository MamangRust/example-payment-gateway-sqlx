use crate::{
    abstract_trait::transfer::repository::stats::amount::TransferStatsAmountRepositoryTrait,
...
pub struct TransferStatsAmountRepository {
    db: ConnectionPool,
}

impl TransferStatsAmountRepository {
...
#[async_trait]
impl TransferStatsAmountRepositoryTrait for TransferStatsAmountRepository {
    async fn get_monthly_amounts(
        ...
        let rows = sqlx::query(sql)
            ...
            .await
            .map_err(|e| {
                error!("❌ Database error in get_monthly_transfer_amounts: {e:?}");
    ...
    async fn get_yearly_amounts(
        ...
        let rows = sqlx::query(sql)
            ...
            .await
            .map_err(|e| {
                error!("❌ Database error in get_yearly_transfer_amounts: {e:?}");
                RepositoryError::Sqlx(e)
            })?;
