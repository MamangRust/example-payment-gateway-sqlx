...
use genproto::withdraw::withdraw_service_server::WithdrawServiceServer;
use shared::{
...
use withdraw::{
    config::ServerConfig,
    service::{WithdrawServiceImpl, WithdrawStats, WithdrawStatsByCard},
    state::AppState,
};
...
async fn main() -> Result<()> {
    ...
    let _logger = Logger::new("withdraw-service", is_dev);
    ...
    let stats = WithdrawStats {
        amount: state.di_container.withdraw_stats.amount_service.clone(),
        status: state.di_container.withdraw_stats.status_service.clone(),
    };

    let statsbycard = WithdrawStatsByCard {
        amount: state
            .di_container
    ...
    let service = WithdrawServiceImpl::new(
        state.di_container.withdraw_query.service.clone(),
        state.di_container.withdraw_command.service.clone(),
        stats,
        statsbycard,
    );
    ...
    info!("✅ Withdraw Service shutdown complete.");
    Ok(())
}
...
async fn start_grpc_server(
    ...
    tonic::transport::Server::builder()
        .add_service(WithdrawServiceServer::new(service))
